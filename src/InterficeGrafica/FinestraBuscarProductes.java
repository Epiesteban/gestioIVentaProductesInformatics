...
public class FinestraBuscarProductes extends JFrame{
	...
	public FinestraBuscarProductes() {
		...
		textField = new JTextField("Busca el producte que vulguis", 40); //Aqui ficarem la busqueda --> lletra x lletra anir� eliminant productes 
		textField.setBackground(new Color(204, 204, 204));
		textField.setForeground(new java.awt.Color(102, 102, 255));
		//Busquem lletra per lletra el nom del producte 

		//Afegim una acci� al JTextField per a que al polsar <enter> sapigui que s'ha acabat d'escriure
		...
		JButton botoCerca = new JButton("CERCA AMB FILTRES"); //El click cercar� els productes amb filtres inclosos
				...
				new WindowAdapter() {
					...
					public void windowClosing(WindowEvent e) {
						...
						for (int i = 0; i < myModel.getSize(); i++) {
							...
							if (checked = false) {
								//no hi ha cap check ficat, per tant la busqueda nomes es definir� per lo que s'escriu al JTextField
							}
							if (checked = true) {
